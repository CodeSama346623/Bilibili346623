...
class TestApp(App):
    def build(self):
        ...
        self.window=Window
        self.window.bind(on_key_up=self.key_action)
...
##        print (args[1],Keyboard().keycode_to_string(args[1]))
        ...
        if args[1]==276:
            ...
            if self.gifindexnow<=(giflistlen-2):
                ...
                print ('down')
                Window.unbind(on_key_up=self.key_action)
                Window.bind(on_key_up=self.key_none)
        self.lab.text=gifFilename[self.gifindexnow]
##        print (self.gifindexnow)
    def key_none(self,*args):
        print("here "+str(args[1]))
